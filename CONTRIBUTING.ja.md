...
>
> 本ファイルは英語版 `CONTRIBUTING.md` の翻訳です。両者に食い違いがある場合は英語版が正本となります。

## ドキュメント言語ポリシー

Nexterm はオープンソースとして世界中で利用されるため、**英語をリポジトリの正本言語**としています。

- ソースコード内のコメント（`//`、`///`、doc コメント、`expect("...")` のメッセージ等）は英語で記述してください。
- リポジトリのドキュメント（`README.md`、`docs/` 配下、`CHANGELOG.md`、ADR 等）は英語版を一次ソースとして作成します。日本語訳が必要な場合は、英語ファイルの隣に `*.ja.md` を置いてください（例: `README.md` + `README.ja.md`）。日本語版はベストエフォートの翻訳として扱い、内容が食い違うときは英語版が正です。
- アプリ内のユーザー向け文字列は `nexterm-i18n` で管理します。`nexterm-i18n/locales/` 配下の**全 8 言語の JSON ファイル**に追加し、必ず `fl!` マクロ経由で参照してください。レンダラに自然言語を直接書かないこと。
- コミットメッセージ・PR タイトル・GitHub Releases のリリースノートは英語を基本としてください。日本語の補足は歓迎しますが、英語の代わりにしないでください。
