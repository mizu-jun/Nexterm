...
- **Status:** P0, P1 and P2 all implemented and merged (2026-09-25/26, PRs #116, #117,
  and a third P2 PR). Only P3 items and the Windows-hardware-gated part of #18 remain
  open, by design. See "Progress" below for what shipped, what turned out to already be
  fixed, and what was deferred with a documented reason.
...
|---|---|---|---|---|---|---|
| 1 | P2 | M | ✅ Done (P2 PR) | Single-binary `CloseAction::Detach` is effectively a kill (server thread dies with the process); the code's own comment admits this | `nexterm-client-gpu/src/renderer/event_handler/window.rs`, `main.rs`, `nexterm-server/src/ipc/platform.rs` | Corrected the misleading "Detach (keep the session)" settings string across all 8 locales; added a `socket_has_a_live_listener` probe to `serve_unix()` so a second server refuses to start (and hijack the socket) rather than silently orphaning a running one — a real single-instance guard, not just detach/reattach prep |
...
| 9 | P2 | S | ✅ Done (P2 PR) | ADR-0005's rejection of a Zellij-style tiling array cites "resize logic gets complex" — Zellij's actual complexity is specifically N-ary percentage redistribution, a problem Nexterm's strictly-binary tree structurally avoids | `docs/adr/0005-*.md` | Added the clarifying note to the ADR's Alternatives section |
| 10 | P2 | S | ✅ Done (P2 PR) | The open macOS-only `chrome_advance` `inf`-for-CJK bug matches a known **class** of bug (Ghostty `#8712`, Alacritty `PR #1029`, WezTerm `#614`) rather than being unique to Nexterm, but root cause is still unknown | `nexterm-client-gpu/src/font.rs` | `chrome_advance` now logs (trace level) the resolved face/family, requested family, bold and size when it hits a non-finite advance, via `cosmic-text`'s `font_id` -> `fontdb` face lookup |
...
| 16 | P2 | S | ✅ Done (P2 PR) | The tmux+TPM zero-sandbox baseline (unconditional full-account shell execution) is a legitimate comparison quantifying what Nexterm's WASM boundary buys, but currently only exists implicitly across several ADRs | `docs/PRODUCT.md`, `examples/plugins/README.md` | Added the explicit paragraph to both files |
| 17 | P2 | S | ✅ Done (P2 PR) | Zellij migrated wasmer → wasmtime → wasmi (2026-03-23); Nexterm is pinned two major versions behind on wasmi (0.38 vs. upstream 2.0.0) | `nexterm-plugin/Cargo.toml` | Documented the version gap in a comment next to the dependency, tied to the existing dependency-review rotation (russh/lru) rather than a one-off bump |
...
| 20 | P2 | M | ✅ Done, partial scope (P2 PR) | `nexterm-ctl` only prints locale-dependent fixed-width text via `fl!`; no scriptable structured output exists | `nexterm-ctl` | Added `nexterm-ctl list --json` (NDJSON via `SessionInfo`'s existing `Serialize`). **Scope note:** only `list` gained this; `new`/`attach`/`kill`/`record`/`theme`/`template`/`plugin`/`workspace` still print `fl!` text only — extend incrementally if scripting demand shows up for those |
| 21 | P2 | S | ✅ Done (P2 PR) | Windows Terminal's Monarch/Peasant model is a different product concept (window/command routing, not a PTY-holding daemon) — not a security gap, but the reason Nexterm accepts a different attack surface is undocumented | `docs/THREAT_MODEL.md` | Added the note to Boundary 1's assessment. **Also found and fixed while here:** the Windows named-pipe DACL row still claimed an unimplemented mitigation as existing — a doc-drift previously reported fixed in an earlier round that never actually landed; moved to Residual risk with a pointer to item #18 |
...
- **P2 (#1, #9, #10, #16, #17, #20, #21) — done, PR on top of #116/#117.** #1 delivered
  more than originally scoped (a real single-instance guard, not just prep for one) and
  #21's doc pass caught a second, still-live doc-drift bug in `THREAT_MODEL.md` (the
  Windows DACL claim) while editing the same table row. #20 is intentionally partial —
  only `nexterm-ctl list` gained `--json`, not every subcommand.
- **Remaining:** P3 (#2, #14, #15, #22) deferred by design — revisit only when the
  blocking feature or need materializes. #18 ②/③ blocked on real Windows hardware.

## Suggested execution order (remaining work)

All P0/P1/P2 work is done. What's left is deliberately deferred:

1. **#18 ②/③, Windows-only, needs real hardware** — schedule once Windows dev/CI access
   is available for iterative compile-error round-tripping; needs a redesign of ① first
   that can tell "attacker squatting" apart from "second legitimate instance" (see table
   row #18 for why the originally-scoped ① was rejected).
2. **#20, extend to other subcommands** — if scripting demand shows up for
   `new`/`attach`/`kill`/`record`/`theme`/`template`/`plugin`/`workspace`, add `--json`
   to those following the same pattern as `list`.
3. **P3** (#2, #14, #15, #22) — do not schedule; revisit only when the blocking feature
...
4. **N/A** (#4, #11, #12) — no further action; kept in this table only for the "21
