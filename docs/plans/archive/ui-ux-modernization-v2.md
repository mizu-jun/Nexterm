...
| 2c | **shipped** | Per-tab Nerd Font process icon. Server-side `Pane::foreground_process_name` (Linux `/proc/{tpgid}/comm`, macOS `ps` parent-tree walk, Windows `Toolhelp32Snapshot`); session-wide 1 Hz polling ticker that diffs against `last_seen` and broadcasts `ServerToClient::ProcessChanged` only on change; `RuntimeConfig.tab_bar.show_process_icon` gates the polling so disabled = zero cost. **PROTOCOL_VERSION 8 → 9** for the new IPC variant. Client `PaneState.process_name` + pure `glyph_for_process` map (~30 entries: editors, network, source control, runtimes, containers, shells); tab bar prepends the glyph when `show_process_icon = true` and the map returns `Some`. Full design in `docs/plans/phase-2c-process-icon.md`. 12 new tests across crates. |
...
| 3b | **shipped** | Live theme preview on hover in the settings panel. `SettingsPanel.theme_hover_preview: Option<usize>` is set by `on_cursor_moved` whenever the cursor sits on a `SettingsPanelHit::ThemeColor(idx)` (cleared otherwise); the redraw path overrides `effective_color_scheme` with `ColorScheme::Builtin(index_to_builtin_scheme(idx))` so `DesignTokens` re-derive per frame. Click commits via the existing `ThemeColor` handler and clears the preview, mouse-leave reverts on the next frame, panel close resets the preview. 5 new unit tests pass. |
...
| 5b | **shipped** | Smooth cursor motion via the new `cursor_motion` module (pure `ease_out_cubic` / `interpolate` / `animation_t` + `CursorMotionState`). `WgpuState` keeps a per-pane motion map and `sample_cursor_motion` advances it each frame; the visible position threads through `build_grid_verts*` into the cursor draw call. `PaneRenderCache` gains a `cursor_visual_q` quantised position so the cache rebuilds during animation. `smooth_motion = false` short-circuits to byte-identical pre-Phase-5b output. 9 new unit tests pass. |
| 6 | **shipped** | `[inactive_pane_hsb]` (`hue` / `saturation` / `brightness`) drives the inactive-pane overlay via the pure `InactivePaneHsbConfig::overlay_rgba(t)` helper; spring-animated dim alpha now serves as the `animation_t` input so the focus-change transition is preserved. v1 ignores `hue` (a real hue shift needs a shader pass, deferred to Phase 6b). 8 new unit tests pass. |
| 6b | **shipped** | True HSB hue shift via a **per-cell CPU transform** during vertex construction (no extra shader pass needed — `bg_pipeline` already carries per-vertex colour). New pure helpers in `color_util.rs` (`rgb_to_hsv`, `hsv_to_rgb`, `apply_hsb_multiplier`, `apply_hsb_animated_rgba`); `build_grid_verts_in_rect` takes an `inactive_hsb: Option<(h, s, b, t)>` and runs every cell's bg/fg through the transform on inactive panes. `PaneRenderCache` gains a quantised `inactive_hsb_q` so the cache invalidates each spring frame. The Phase-6 ui_verts overlay is suppressed when HSB is active (would otherwise double-dim). 8 new unit tests pass. |
