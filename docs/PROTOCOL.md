...
フォーカスペインを閉じる（BSP ノード削除・兄弟昇格）。ペインが 1 つしかない場合は `Error` を返す。

```
{ ClosePane }
```

**応答**: `PaneClosed` → `LayoutChanged` または `Error`
...
- `delta > 0`: フォーカスペインを広げる
- `delta < 0`: フォーカスペインを縮める
- 比率は `[0.1, 0.9]` にクランプされる
...
新しいウィンドウ（タブ）を作成し、フォーカスを移動する。

```
{ NewWindow }
```

**応答**: `WindowListChanged` → `FullRefresh` → `LayoutChanged`
...
**応答**: `WindowListChanged` または `Error`
...
**応答**: `WindowListChanged` → `LayoutChanged` → `FullRefresh`
...
ブロードキャスト入力モードを切り替える。有効時はフォーカスウィンドウの全ペインに同時書き込みする。
...
指定ホストに SSH 接続し、新規ペインで PTY チャネルを開く。
...
**応答**: `FullRefresh` → `LayoutChanged` または `Error`
...
ペインが閉じられたことを通知する。クライアントはレンダリングからペインを削除する。
...
ウィンドウ一覧の変更通知（作成・削除・リネーム・フォーカス変更時）。
...
OSC 0/2 による端末タイトル変更通知。クライアントはウィンドウタイトルまたはタブラベルを更新する。
...
OSC 9 による通知。クライアントは OS のデスクトップ通知を表示する。
