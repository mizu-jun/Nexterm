...
| `ShowSettings` | 設定 GUI パネルを開く（`Ctrl+,` デフォルト） |
...
| `x11_forward` | bool | `false` | X11 フォワーディングを有効にする（`ssh -X` 相当） |
| `x11_trusted` | bool | `false` | 信頼された X11 フォワーディング（`ssh -Y` 相当、`x11_forward` より優先） |
...
ブラウザから接続できる組み込み Web ターミナル。デフォルト無効。

| キー | 型 | デフォルト | 説明 |
|-----|----|-----------|------|
| `enabled` | bool | `false` | Web ターミナルを有効にする |
| `port` | u16 | `7681` | リッスンポート |
| `token` | String | — | アクセストークン（省略時は認証なし。LAN 限定推奨） |
...
> **セキュリティ注意**: `token` を設定しない場合、LAN 内の全端末からアクセス可能になります。
> デフォルトのバインドアドレスは `0.0.0.0`（全インターフェース）です。
> ローカルのみで使用する場合は `token` を必ず設定してください。
