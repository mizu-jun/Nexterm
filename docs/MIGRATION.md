...
## v1.5.1 → Unreleased（Sprint 5-11-8 Step 8-3: SSH ホスト GUI 編集 + アクセシビリティ拡充）

**互換性破壊なし**。`PROTOCOL_VERSION = 8` / `SNAPSHOT_VERSION = 4` のまま、設定パネルの
SSH カテゴリに GUI 編集機能とスクリーンリーダー対応を追加しました。既存ユーザーは何もせず
アップグレード可能です。
