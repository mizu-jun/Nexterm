...
- タブラベルに OSC 0/2 ウィンドウタイトル（例: カレントディレクトリ）が表示されるようになり、TUI クライアントと統一。
- タブバー右側に「⚙ Settings」ボタンを追加。クリックで設定パネルを開閉可能。
- タブをクリックしてペインを切り替え可能（キーボードショートカット不要）。
- 設定パネルのフォントファミリーフィールドが完全編集可能に: **F** キーで編集モード開始、**Backspace** で削除、**Enter** で確定、**Escape** でキャンセル。

**ドキュメント**
- 全ドキュメントを英語を主言語に統一。
- シェーダー・パフォーマンス・グラフィックス・プラグインの日本語訳を追加。
...
**カスタム WGSL シェーダー**
- `nexterm.toml` の `[gpu]` セクションで `custom_bg_shader` / `custom_text_shader` のパスを指定。
- CRT スキャンライン・グローテキストなどビジュアルエフェクトを WGSL で実装可能。
- ファイル読み込み失敗時はビルトインシェーダーに自動フォールバック。

**GPU レンダラーのパフォーマンス向上**
- 頂点・インデックスバッファをフレーム間で**再利用**（`COPY_DST` + `write_buffer`）。
  毎フレーム 4 回の GPU アロケーションが**0 回**に削減。
- ASCII 印字可能文字（0x20–0x7E）を起動時にグリフアトラスへ**事前ロード** — 初回キーストローク遅延を解消。
- `gpu.fps_limit`（デフォルト 60）でフレームレートを上限設定し、アイドル時の CPU/GPU 負荷を低減。

**起動時間の改善**
- ランチャーの `wait_for_server` ポーリングを固定 100ms から**指数バックオフ**（10ms→100ms）に変更。
  サーバー検出の平均待機時間が約 100ms → 約 30ms に短縮。

**ドキュメント充実**
- Sixel/Kitty グラフィックスガイド、WASM プラグイン開発ガイド、カスタムシェーダーリファレンス、パフォーマンスチューニングガイドを追加。
...
- **デーモンレス設計** — `nexterm` シングルバイナリ内部の tokio タスクが PTY を保持し、クライアント再接続時にセッションを復元（v0.9.3 以降。systemd など独立プロセスで運用したい場合は `nexterm-server` を別途配布）
...
- **コマンドブロック（Warp 風）** — OSC 133 A/B/C/D を検出してプロンプト〜コマンド〜出力〜終了コードを `CommandBlock` として束ね、専用 UI を提供
  - **ブロックナビゲーション** — `Ctrl+Shift+ArrowUp/ArrowDown` で前後のプロンプトへジャンプしつつブロックを選択
  - **ブロックコピー** — ブロック選択中は `Ctrl+Shift+C` がコマンド + 出力をまとめてクリップボードへ（未選択時は既存挙動の全画面コピーにフォールバック）
  - **ブロック再実行** — `Ctrl+Shift+R` で選択ブロックのコマンドを再投入。ESC / BEL / CSI / 改行混入は防御的サニタイザで拒否
  - **名前付きブロック** — `Ctrl+Shift+L` でリネームモーダル。`~/.local/state/nexterm/named_blocks.json` に atomic write + 0600 で永続化、上限 10 000 件
  - **視覚オーバーレイ** — スクロールバック表示時に左 border を成功=緑 / 失敗=赤 / 実行中=灰で描画、選択ブロックには行幅 tint。バッジ glyph と通常 grid 表示は実機検証後の続編で
  - **互換性** — WezTerm / kitty / Ghostty と同じ OSC 133 を共有。bash / zsh / fish 用の PS1 スニペットは [`docs/shell-integration.md`](docs/shell-integration.md) 参照
...
| コンポーネント | 内容 | 状態 |
|--------------|------|------|
| `nexterm-proto` | IPC プロトコル型定義 (postcard) | ✅ |
...
| W-3 | `nexterm.exe` 単一エントリーポイント（サーバーは内部 tokio タスクとして起動） | ✅ |
...
1. [Releases](https://github.com/mizu-jun/nexterm/releases) ページから `nexterm-vX.Y.Z-windows-x86_64.msi` をダウンロードする。
...
├── nexterm-client-tui    # TUI クライアント (ratatui + crossterm)
├── nexterm-client-gpu    # GPU クライアント (wgpu + winit + cosmic-text)。ビルド成果は `nexterm` バイナリ — サーバーを内部 tokio タスクとして起動
...
- Rust 1.85 以上（workspace の `edition = "2024"` が必須とするため）
